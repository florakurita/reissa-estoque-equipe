...
# ============================================================
    ...
    with col2:
        if st.button("🏪 Estoque\nMezanino", use_container_width=True, type="primary"):
            ...
            st.session_state.dados_lev = {}
            st.rerun()
        if st.button("↩️ Registrar\nDevolução", use_container_width=True):
            st.session_state.tela = "devolucao"
            st.rerun()
...
# ============================================================
    ...
    # Filtrar apenas códigos que têm inventário nesse tamanho
    if not df_inv.empty and codigos_cluster:
        ...
        # Também incluir códigos antigos via de-para
        todos_codigos_com_inv = set()
        for cod in codigos_cluster:
            todos_vinculados = get_codigos_vinculados(cod, df_depara)
            for c in todos_vinculados:
                mask_inv = (df_inv["Código"] == c) & (df_inv["Tamanho"] == d["tamanho"])
                if mask_inv.any():
                    todos_codigos_com_inv.add(cod)
                    break
        
        codigos_filtrados = [c for c in codigos_cluster if c in todos_codigos_com_inv]
    else:
...
# REGISTRAR DEVOLUÇÃO
# ============================================================
elif st.session_state.tela == "devolucao":
    st.title("↩️ Registrar Devolução")
    if st.button("← Voltar ao menu"):
        st.session_state.tela = "menu"
        st.rerun()
    ...
    with st.form("form_devolucao"):
        data_dev = st.date_input("📅 Data da devolução", value=date.today())
        codigo   = st.text_input("Código da calça")
        if codigo and not df_cad.empty:
            df_cad["Código"] = df_cad["Código"].apply(lambda x: str(int(float(str(x)))) if str(x).replace('.','').isdigit() else str(x))
            linha = df_cad[df_cad["Código"] == codigo]
            if not linha.empty:
                info  = linha.iloc[0]
                grupo = str(info.get("Grupo Tamanho", "REG"))
                st.info(f"✅ {info.get('Nome (auto)', '')} — {grupo}")
            else:
                grupo = "REG"
        ...
        tams = tamanhos_por_grupo(grupo)
        st.markdown("**Quantidade por tamanho:**")
        cols = st.columns(len(tams))
        quantidades = {}
        for i, tam in enumerate(tams):
            with cols[i]:
                st.markdown(f"**{tam}**")
                qtd = st.number_input(f"d{tam}", min_value=0, value=0, step=1,
                                      label_visibility="collapsed")
                quantidades[tam] = qtd

        motivo  = st.text_input("Motivo (opcional)")
        nf      = st.text_input("Nº NF devolução (opcional)")
        salvar  = st.form_submit_button("↩️ Registrar devolução", type="primary", use_container_width=True)

    if salvar and codigo:
        try:
            data_str = data_dev.strftime("%d/%m/%Y")
            for tam, qtd in quantidades.items():
                if qtd > 0:
                    linha = [data_str, "Devolução", int(codigo), "", grupo, str(tam), qtd, "", "", "", nf]
                    append_row("Compras", linha)
